...
def test_binary_csv_reader(buffer_size):
    source = StringIO(
        '"first","text","last"\n,"comma, quote"" and\r\nnewline",""\n"00 ff","日本語",\n'
    )
    with TextIOWrapper(
...
def test_binary_csv_reader_close_before_eof():
...
def test_binary_csv_reader_preserves_other_fields():
    source = StringIO('"binary","text"\r\n,unquoted\r\n"",\r\n')
    with BinaryCSVReader(source, {0}) as stream:
        assert stream.read().decode() == (f'"binary","text"\r\n{_BINARY_NULL},unquoted\r\n"",\r\n')
