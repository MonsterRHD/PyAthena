...
def test_wrangle_options_pass_actual_library_validation(monkeypatch, transport):
    calls = []
...
def test_all_cursor_factories_accept_the_selected_options(monkeypatch):
...
async def test_real_api_cursors_consume_stubbed_athena_results(api):
    session = boto3.Session(
        aws_access_key_id="testing", aws_secret_access_key="testing", region_name="us-west-2"
    )
            ...
            stubber.add_response(
                "get_query_results",
                {
                    "ResultSet": {
                        ...
                        "Rows": [
                            {"Data": [{"VarCharValue": value}]} for value in ("a", "1", "2", "3")
                        ],
                    }
                },
            )
...
@pytest.mark.parametrize("api", ["sync", "thread", "aio"])
async def test_each_api_consumes_all_rows_and_reports_boundaries(api):
    adapter = Adapter(
...
async def test_wrong_row_count_is_not_success():
...
async def test_aio_native_consumption_is_offloaded(monkeypatch):
    calls = []
...
async def test_concurrent_aio_uses_distinct_cursors(monkeypatch):
    active = 0
    peak = 0
    created = []
